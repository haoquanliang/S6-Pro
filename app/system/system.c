...
static void ntc_charge_enter_0p3c(void)
{
    ...
    {
        if (!ntc_charge_0p3c_enable)
        {
            printf("ntc_charge_enter_0p3c\n");
            charge_stop(0);
            charge_0p3c_init();
            ntc_charge_0p3c_enable = true;
        }
    }
}

static void ntc_charge_exit_0p3c(void)
{
    if (xcfg_cb.ntc_0p3c_en)
    {
        if (ntc_charge_0p3c_enable)
        {
            printf("ntc_charge_exit_0p3c\n");
            charge_stop(0);
            charge_init();
            ntc_charge_0p3c_enable = false;
        }
    }
}
...
#if USER_RECHARGE
        if (sys_cb.flag_recharge_complete == 1)
        {
            if (cc_cnt++ >= 1000) //5秒转换状态
            {
                cc_cnt = 0;
                sys_cb.flag_recharge_complete = 2;
                RTCCON8 = (RTCCON8 & ~BIT(1)) | BIT(6);// 重新使能充电,enable charger function
            }
        }
